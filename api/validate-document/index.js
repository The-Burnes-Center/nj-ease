...
module.exports = async function (context, req) {
  context.log('Document validation function triggered');
  ...
  try {
    ...
    // Extract form fields
    let file = null;
    let documentType = "tax-clearance-online";
    ...
    for (const part of parts) {
      ...
      } else if (part.name === 'documentType') {
        documentType = part.data.toString();
      } else if (part.name === 'organizationName') {
        organizationName = part.data.toString();
      } else if (part.name === 'fein') {
        fein = part.data.toString();
      }
    }
    ...
    const poller = await client.beginAnalyzeDocument("prebuilt-document", file.data, {
    ...
    // Validate based on document type (simplified version)
    ...
    context.res = {
      ...
      status: 500,
      body: {
        error: error.message === "Processing timeout" 
          ? "Document processing timed out. Please try again with a smaller file or check your document quality."
